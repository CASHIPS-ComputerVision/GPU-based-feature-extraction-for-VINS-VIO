...
namespace MultiColSLAM
{
	...
	cMultiFrame::cMultiFrame(const std::vector<cv::Mat>& images_,
		...
		int _imgCnt,
		mdBRIEFextractorCon* pExtractor,
		cReadImage* readImage,
		std::vector<cv::Mat> &images_temp,
		std::vector<std::vector<cv::KeyPoint>> &keyPtsTemp_temp,
		std::vector<cv::Mat> &mDescriptors_temp,
		std::vector<cv::Mat> &mDescriptorMasks_temp)
		:
		...
		imgCnt(_imgCnt)
	{
		...
		if(!pExtractor->isReady(images,keyPtsTemp,mDescriptors,mDescriptorMasks,mp_mdBRIEF_extractorOct[0]))
		{
			while(!pExtractor->isReady(images,keyPtsTemp,mDescriptors,mDescriptorMasks,mp_mdBRIEF_extractorOct[0]))
			{
		...
		for (int c = 0; c < nrCams; ++c)
		{
			...
			mnMinX[c] = 0;
			mnMaxX[c] = camModel[c].GetWidth();
			mnMinY[c] = 0;
			mnMaxY[c] = camModel[c].GetHeight();
