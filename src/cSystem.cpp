...
namespace MultiColSLAM
{
	...
	cSystem::cSystem(const string &strVocFile, const string &strSettingsFile,
		const string& path2MCScalibrationFiles,vector<vector<string>> imgFilenames,
		...
		mbDeactivateLocalizationMode(false)
	{
		...
		//Initialize the Read Image thread and launch
		mpReadImage=new cReadImage(imgFilenames,1);
		mptReadImage=new thread(&cReadImage::Run, mpReadImage);
		
		//Initialize the Extractor thread and launch
		mpExtractor=new mdBRIEFextractorCon(mpReadImage,imgFilenames,camSystem,strSettingsFile,2);
		mptExtractor=new thread(&mdBRIEFextractorCon::Run, mpExtractor);
		...
		mpTracker->SetExtractorCon(mpExtractor);
		mpTracker->SetReadImage(mpReadImage);
	...
	void cSystem::Shutdown()
	{
		...
		mpExtractor->RequestFinish();
		mpReadImage->RequestFinish();
		...
		while (!mpLocalMapper->isFinished() || !mpLoopCloser->isFinished() ||
			!mpViewer->isFinished()||!mpExtractor->isFinished()||!mpReadImage->isFinished())
		{
